...
from transform import group_colorjitter
...
class Ucf101Reader(object):
       ...
       # if set video_path for inference mode, just load this single video
        if (self.mode == 'infer') and (self.video_path != ''):
            ...
            _reader = self._reader_creator(pickle_list=self.filelist, mode = self.mode, seg_num=self.seg_num, seglen = self.seglen, \
                             step=self.step,short_size = self.short_size, target_size = self.target_size, \
    ...
    def _reader_creator(self,
        ...
        def decode_jpg(sample,classInd,vid_path, mode, seg_num, seglen,step, short_size,
                          target_size, img_mean, img_std):
            pickle_path = sample[0].split('/')[1]
            pickle_path = pickle_path.split('.')[0]
            ret_label = classInd[sample[0].split('/')[0]]-1
            jpg_path = os.path.join( vid_path,pickle_path)
            if(not(os.path.exists(jpg_path))):
                logger.error('{} path not exists'.format(jpg_path))
                return None, None
         #读入原始尺寸图片 
            imgs = video_loader(jpg_path, seg_num, seglen,step, mode)
            return imgs_transform(imgs, mode, seg_num, seglen, \
                         short_size, target_size, img_mean, img_std, name = self.name), ret_label
...
def imgs_transform(imgs,
    ...
    #     imgs = group_colorjitter(imgs,0.1)
...
#        imgs = group_multi_scale_crop(imgs, short_size)
    #    imgs = group_RandomResizedCrop(imgs,target_size)
...
def group_corner_crop(img_group,target_size,fix_center=False):
    ...
    def corner_crop(img,fix_center=False):
       ...
       # scales = [1,0.85,0.7,0.6,0.5]
        scales = [1,0.94,0.875,0.81,0.75]
        crop_positions = ['c', 'tl', 'tr', 'bl', 'br']
        #获得随机的尺度和采样位置
        if(fix_center==False):
            crop_position = crop_positions[random.randint(0, len(crop_positions) - 1)]
            crop_scale = scales[random.randint(0, len(scales) - 1)]
        else:
    ...
    ret_img_group = [
        img.resize((target_size, target_size), Image.BILINEAR)
...
def group_RandomResizedCrop(img_group, target_size, scales=0.9):
    scale = np.random.uniform(scales,1)
    input_size = [target_size, target_size]

    w,h = img_group[0].size
    base_size = min(w,h)
    crop_sizes = int(base_size*scale)
    crop_img_group = group_random_crop(img_group,crop_sizes)
...
def video_loader(path, nsample, seglen,step, mode):
